...
- Base de datos: mediante MONGO_URI proporcionada o MongoDB local en contenedor.
...
- Docker (opcional, para levantar MongoDB local)
- Cuenta en Render (solo si vas a desplegar)

Inicio rápido
Este proyecto está preparado para que un revisor lo ejecute localmente sin crear su propia cuenta de MongoDB. Yo proporcionaré por privado una MONGO_URI preconfigurada (usuario temporal con acceso a una base de revisión en mi cluster). Con esa cadena:

- Todas las escrituras irán a la base de revisión de mi cluster (aislada de datos productivos).
- No necesitas registrar ni configurar tu propio cluster.
...
3. Configura la base de datos (elige una):
   - Opción A: MONGO_URI proporcionada (recomendada para revisión)
     - Usar la cadena compartida de forma privada.
...
- Run (usando MONGO_URI proporcionada):
  docker run --rm -p 3000:3000 \
   -e PORT=3000 \
   -e MONGO_URI="mongodb+srv://challenge_reviewer:PASSWORD_URLENCODED@TUCLUSTER.mongodb.net/message_counter_review?retryWrites=true&w=majority" \
...
- Mantén MONGO_URI como secreto; no lo subas al repo. Si usas .env, está ignorado en [.gitignore](.gitignore:2).
- Si expones el servicio públicamente, considera:
  - Rate limiting y validaciones (mejora futura: firma HMAC del webhook).
  - Rotación de credenciales y evitar 0.0.0.0/0 en producción.
